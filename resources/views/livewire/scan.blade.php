        ...
        <div class="flex">
          ...
          <div class="ml-3">
            <h3 class="text-sm font-medium text-blue-800 dark:text-blue-200">
              Panduan Penggunaan Scanner
            </h3>
            <div class="mt-2 text-sm text-blue-700 dark:text-blue-300">
              <ul class="list-disc space-y-1 pl-5">
                <li>Pastikan browser mengizinkan akses kamera dan lokasi</li>
                <li>Untuk smartphone: Aktifkan lokasi di pengaturan perangkat</li>
                <li>Jika menggunakan Chrome: Klik ikon gembok di address bar → Izinkan kamera dan lokasi</li>
                <li>Aplikasi memerlukan HTTPS untuk berfungsi optimal di perangkat mobile</li>
              </ul>
            </div>
          </div>
        </div>
...
@script
  <script>
    ...
    // Cek apakah halaman menggunakan HTTPS
    if (location.protocol !== 'https:' && location.hostname !== 'localhost' && location.hostname !== '127.0.0.1') {
      errorMsg.innerHTML = 'Aplikasi memerlukan HTTPS untuk mengakses kamera dan lokasi. Silakan gunakan HTTPS atau localhost.';
    }
    ...
    async function getLocation() {
      if (navigator.geolocation) {
        ...
        navigator.geolocation.watchPosition((position) => {
          ...
          alert(errorMessage);
          document.querySelector('#scanner-error').innerHTML = errorMessage;
        }, options);
      } else {
        document.querySelector('#scanner-error').innerHTML = "Browser tidak mendukung geolocation";
      }
    }
    ...
    if (!$wire.isAbsence) {
      ...
      async function startScanning() {
        ...
        try {
          // Coba akses kamera dengan permission yang eksplisit
          ...
          await scanner.start({
            ...
            (errorMessage) => {
              ...
              if (errorMessage.includes('Permission') || errorMessage.includes('NotAllowedError')) {
                errorMsg.innerHTML = 'Akses kamera ditolak. Silakan aktifkan izin kamera di pengaturan browser.';
              } else {
                errorMsg.innerHTML = 'Gagal memulai scanner: ' + errorMessage;
              }
            }
          );
        } catch (error) {
          ...
          if (error.name === 'NotAllowedError') {
            errorMessage = 'Akses kamera ditolak. Silakan aktifkan izin kamera di pengaturan browser.';
          } else if (error.name === 'NotFoundError') {
            errorMessage = 'Kamera tidak ditemukan pada perangkat ini.';
          } else if (error.name === 'NotSupportedError') {
            errorMessage = 'Browser tidak mendukung akses kamera.';
          } else {
            errorMessage = 'Gagal mengakses kamera: ' + error.message;
          }
